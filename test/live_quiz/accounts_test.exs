defmodule LiveQuiz.AccountsTest do
  ...
  alias LiveQuiz.Accounts.{User, UserToken}
  ...
  describe "register_user/1" do
    ...
    test "validates email when given" do
      ...
      assert %{email: ["precisa ter o sinal @ e não pode conter espaços"]} = errors_on(changeset)
    end
    ...
    test "validates password confirmation when given" do
      {:error, changeset} =
        Accounts.register_user(valid_user_attributes(password_confirmation: "another one"))
  ...
  describe "change_user_registration/2" do
    ...
    test "allows fields to be set without hashing the password" do
      email = unique_user_email()
      ...
      assert get_change(changeset, :name) == "Ana"
  ...
  describe "update_user_password/2" do
    ...
    test "validates password", %{user: user} do
      ...
      assert %{
               ...
               password_confirmation: ["não confere com a senha"]
             } = errors_on(changeset)
    end
  ...
  describe "deliver_user_confirmation_instructions/2" do
    ...
    test "does not send anything for an already confirmed account" do
      user = user_fixture()

      assert Accounts.deliver_user_confirmation_instructions(user, & &1) ==
  ...
  describe "confirm_user/1" do
    ...
    test "confirms the account with a valid token", %{user: user, token: token} do
    ...
    test "keeps the active sessions", %{user: user, token: token} do
      session_token = Accounts.generate_user_session_token(user)
      assert {:ok, _user} = Accounts.confirm_user(token)
      assert Accounts.get_user_by_session_token(session_token)
    end

    test "does not confirm with an invalid token", %{user: user} do
      assert Accounts.confirm_user("oops") == :error
      refute Repo.get!(User, user.id).confirmed_at
    end

    test "does not confirm with an expired token", %{user: user, token: token} do
      {1, nil} = Repo.update_all(UserToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
  ...
  describe "get_user_by_reset_password_token/1" do
    ...
    test "does not return the user if the token expired", %{token: token} do
      {1, nil} = Repo.update_all(UserToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
